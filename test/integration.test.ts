...
describe('real DSH composition', () => {
  it('registers commands and produces a read-only repair prompt from a live session', async () => {
    ...
    const next = await ctx.commands.execute(agent, '/postmortem-next 1', [], new AbortController().signal)
    ...
    expect(next?.result.text).toContain('Primary issue: Tool shell failed')
    expect(next?.result.text).toContain('/postmortem-handoff 1')
    expect(next?.result.text).toContain('project verifier, test command, or checker')
    expect(next?.result.text).not.toContain('does-not-exist')
    expect(next?.result.text).not.toContain('private output')
  ...
  it('reports an open scheduled model retry without calling a review model', async () => {
    ...
    const next = await ctx.commands.execute(agent, '/postmortem-next', [], new AbortController().signal)
    ...
    expect(next?.result.text).toContain('Wait: this turn has no repair action')
    expect(next?.result.text).not.toContain('/postmortem-recover')
  })
})
