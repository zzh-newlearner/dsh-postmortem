...
describe('diagnose', () => {
  ...
  it('finds unchanged repeated failures', () => {
    const report = diagnose({ sessionId: 's1', turn: 1, sourceSeq: 4, ended: true, endReason: 'error', toolCalls: [
      { callId: '1', name: 'bash', step: 1, arguments: '{"cmd":"bad"}', isError: true, resultPresent: true },
      { callId: '2', name: 'bash', step: 2, arguments: '{"cmd":"bad"}', isError: true, resultPresent: true },
      { callId: '3', name: 'bash', step: 3, arguments: '{"cmd":"bad"}', isError: true, resultPresent: true },
    ] })
